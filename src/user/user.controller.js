...
class user_controller extends user_module {
  static create_user = async (req, res) => {
    try {
      ...
      let status_code =
        error.status.code != undefined ? error.status_code : 500;
      let type = error.type != undefined ? error.type : "Bad Request";
      let message =
        err.custom_msg != undefined ? error.custom_msg : "Something went wrong";
  ...
  static get_users = async (req, res) => {
    try {
      console.log("controller response", req.body);
      let response = await this.retreive_user(req);
      let message = "Success";
      res.send({
        success: true,
        message: message,
